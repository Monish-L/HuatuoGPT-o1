"""
Modified Training Script for Solution Guidance Methodology
==========================================================
This script adapts the HuatuoGPT-o1 training approach for medical reasoning
using the Solution Guidance framework.

Key Modification: Changed prompt format from generic "Thinking" to medical-focused "Solution Guidance"

Author: [Your Name]
Purpose: Masters Thesis - Applying Solution Guidance to Medical Domain
"""
...
class Train_dataset(torch.utils.data.Dataset):
    def __init__(self, config, tokenizer):
        ...
        for da in self.data:
            newdata.append(da)
        print(f'Loaded {len(self.data)} training samples, using {len(newdata)} samples')
        self.data = newdata
    ...
    def get_response(self, da):
        """
        MODIFIED FOR SOLUTION GUIDANCE METHODOLOGY
        ...
        New format (Solution Guidance):
        ## Solution Guidance
        {step-by-step medical problem decomposition}

        ## Final Response
        {clinical answer}

        This change emphasizes medical problem-solving guidance
        rather than generic reasoning.
        """
        # ORIGINAL: temp = '## Thinking\n\n{}\n\n## Final Response\n\n{}'
        # MODIFIED FOR THESIS:
...
def train(args):
    ...
    accelerator.print(f'Training with Solution Guidance Methodology')
    ...
    accelerator.print(f'  - Epochs: {args.n_epochs}')
    ...
    def save_checkpoint(epoch, step, global_step):
        ...
        if accelerator.is_main_process:
            ...
            print(f'Model saved in {output_dir}, copied files: {copy_files}')

        if accelerator.state.deepspeed_plugin.zero_stage == 3:
            unwrap_model = accelerator.unwrap_model(model)
            unwrap_model.save_pretrained(
                os.path.join(save_dir, f'tfmr'),
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Solution Guidance Fine-Tuning for Medical Reasoning')

    # Experiment Args
    parser.add_argument('--experiment_name', type=str, default='solution_guidance_medical')

    # Model Args
    parser.add_argument('--model_path', required=True, type=str, help='Path to base model (e.g., meta-llama/Llama-3.1-8B-Instruct)')

    # Data Args
    parser.add_argument('--data_path', required=True, type=str, help='Path to training data JSON')
    ...
    print("=" * 60)
    print("SOLUTION GUIDANCE TRAINING")
    print("Masters Thesis - Medical Reasoning")
    print("=" * 60)
